...
- Raspberry PI:
    1. Testing:
       - Arducam OV5647 camera module with built-in motorized IR-CUT filter attached via CSI.
         ...
         3. Front-End: Basic streaming and a table that shows the objects detected to the user. The table is important so that the operator can choose which object the ROV has to move to.
...
- Local Server & VPS:
  1. Testing:
     - To keep the testing scalable and especially reduce the amount of work of the raspberry (since it goes at 2.5 fps with yolo11n_ncnn), In this part I will use a local machine (an old HP) that simulates the behaviour of the vps.<br>
